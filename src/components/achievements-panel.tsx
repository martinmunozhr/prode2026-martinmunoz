  ...
  useEffect(() => {
    (async () => {
      ...
      const [coinsRes, streakRes, txsRes, payoutsRes] = await Promise.all([
        ...
        supabase
          .from("user_streaks")
          .select("exact_streak, best_exact_streak")
          .eq("user_id", userId)
          .maybeSingle(),
        supabase
        ...
        supabase
          .from("round_payouts")
          .select("round_id, points_paid, achievements_paid, paid_at")
          .eq("user_id", userId)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          icon={Crown}
          label="Mejor racha"
          value={streak?.best_exact_streak ?? 0}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <MiniStat
          label="Total gastado"
          value={`-${coins?.total_spent ?? 0}`}
          accent="text-muted-foreground"
        />
        <MiniStat
          label="Saldo neto"
          value={String((coins?.total_earned ?? 0) - (coins?.total_spent ?? 0))}
        />
      </div>
      ...
      <div className="rounded-2xl border border-border/50 bg-card/40 p-5">
        ...
        {payouts.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Aún no se cerró ninguna fecha. Una vez que termine, vas a ver acá tus logros.
          </p>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="font-bold text-sm uppercase tracking-wider">
                      {p.round_id.replace(/-/g, " ")}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {p.points_paid} pts → +{p.points_paid * 100} monedas
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <AchievementBadge
                      active={Number(ach.exact_streak) > 0}
                      icon={Flame}
                      label={`Racha exactos +${ach.exact_streak ?? 0}`}
                    />
                    <AchievementBadge
                      active={Number(ach.goalscorer) > 0}
                      icon={Zap}
                      label={`Goleadores +${ach.goalscorer ?? 0}`}
                    />
                    <AchievementBadge
                      active={Number(ach.perfect) > 0}
                      icon={Target}
                      label="Pleno de fecha +500"
                    />
                    <AchievementBadge
                      active={Number(ach.top1) > 0}
                      icon={Crown}
                      label="#1 de la fecha +1000"
                    />
                  </div>
                </div>
              );
            ...
            {txs.map((t) => {
              ...
              return (
                <li key={t.id} className="flex items-center gap-3 py-2.5">
                  <div
                    className={cn(
                      "h-8 w-8 rounded-lg bg-background/60 flex items-center justify-center",
                      meta.color,
                    )}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] uppercase tracking-widest text-muted-foreground">
                      {meta.label} · {new Date(t.created_at).toLocaleString()}
                    </div>
                  </div>
                  <div
                    className={cn(
                      "font-display text-lg tracking-wider",
                      positive ? "text-pitch" : "text-muted-foreground",
                    )}
                  >
                    {positive ? "+" : ""}
                    {t.amount}
  ...
  return (
    <span
      className={cn(
        ...
        active
          ? "bg-pitch/10 text-pitch border-pitch/40"
          : "bg-muted/20 text-muted-foreground border-border/40 opacity-60",
      )}
    >

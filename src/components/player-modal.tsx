...
const rarityMeta: Record<
  Player["rarity"],
  { label: string; color: string; stars: number; gradient: string }
> = {
  common: {
    label: "Común",
  ...
  legendary: {
    ...
    color: "text-primary",
    stars: 4,
    gradient: "from-primary/60 via-accent/40 to-alert/40",
  },
};
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0 overflow-hidden border-2 border-primary/30 bg-card">
        ...
        <div
          className={cn(
            "relative aspect-[3/4] bg-gradient-to-br p-6 flex flex-col",
            r.gradient,
            isLegendary && "bg-gradient-holo animate-holo",
          )}
        >
          {isLegendary && (
            <div className="absolute inset-0 opacity-20 mix-blend-overlay pointer-events-none bg-gradient-holo animate-holo" />
          )}
          ...
          <div className="flex items-start justify-between relative z-10">
            ...
            <div
              className={cn(
                "text-[11px] font-bold uppercase tracking-widest px-3 py-1 rounded-full bg-background/50 backdrop-blur",
                r.color,
              )}
            >
              {player.position}
          ...
          <div className="flex-1 flex items-center justify-center relative z-10">
            {teamId && teamIdToIso2(teamId) ? (
              <Flag
                teamId={teamId}
                className={cn(
                  "text-[10rem] !rounded-lg shadow-elevated",
                  isLegendary && "drop-shadow-2xl",
                )}
              />
            ) : (
              <div className={cn("text-[10rem] leading-none", isLegendary && "drop-shadow-2xl")}>
                {teamFlag}
              </div>
            )}
          </div>
          ...
          <div className="relative z-10 text-center">
            <div
              className={cn(
                "font-display text-3xl leading-tight uppercase tracking-wide",
                isLegendary ? "text-background" : "text-foreground",
              )}
            >
              {player.name}
        ...
        <div className="p-5 space-y-4 bg-card">
          <div className="flex items-center justify-center gap-1.5">
            ...
            <span className={cn("ml-2 text-xs uppercase tracking-widest font-bold", r.color)}>
              {r.label}
            </span>
          </div>

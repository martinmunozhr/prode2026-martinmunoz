...
function AdminAwards() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (a)
        setData({
          campeon_id: a.campeon_id,
          subcampeon_id: a.subcampeon_id,
          tercer_puesto_id: a.tercer_puesto_id,
          fair_play_id: a.fair_play_id,
          goleador_nombre: a.goleador_nombre,
          mejor_jugador_nombre: a.mejor_jugador_nombre,
          mejor_arquero_nombre: a.mejor_arquero_nombre,
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="rounded-xl border border-border/50 bg-card/40 p-5">
        ...
        <p className="text-sm text-muted-foreground mb-5">
          Cargá los ganadores cuando termine el Mundial. Esto recalcula automáticamente los puntos
          de Bola de Cristal de todos los usuarios.
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <TeamSelect
            ...
            teams={teams}
            onChange={(v) => setData({ ...data, campeon_id: v })}
          />
          <TeamSelect
            label="Subcampeón"
            value={data.subcampeon_id}
            teams={teams}
            onChange={(v) => setData({ ...data, subcampeon_id: v })}
          />
          <TeamSelect
            label="Tercer Puesto"
            value={data.tercer_puesto_id}
            teams={teams}
            onChange={(v) => setData({ ...data, tercer_puesto_id: v })}
          />
          <TeamSelect
            label="Fair Play"
            value={data.fair_play_id}
            teams={teams}
            onChange={(v) => setData({ ...data, fair_play_id: v })}
          />
          <TextInput
            label="Goleador"
            value={data.goleador_nombre}
            onChange={(v) => setData({ ...data, goleador_nombre: v })}
          />
          <TextInput
            label="Mejor Jugador"
            value={data.mejor_jugador_nombre}
            onChange={(v) => setData({ ...data, mejor_jugador_nombre: v })}
          />
          <TextInput
            label="Mejor Arquero"
            value={data.mejor_arquero_nombre}
            onChange={(v) => setData({ ...data, mejor_arquero_nombre: v })}
          />
          <label className="flex items-end gap-2">
            <input
              type="checkbox"
              checked={data.finalized}
              onChange={(e) => setData({ ...data, finalized: e.target.checked })}
        ...
        <button
          onClick={save}
          disabled={saving}
          className="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-gradient-pitch text-primary-foreground font-bold uppercase tracking-wider text-sm disabled:opacity-50"
        >
      ...
      <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-5">
        ...
        <p className="text-sm text-muted-foreground mt-1 mb-4">
          Recalcula los puntos de TODOS los partidos finalizados. Útil si corregiste un resultado.
        </p>
        <button
          onClick={recalc}
          disabled={recalcing}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-destructive/40 text-destructive text-sm font-bold uppercase tracking-wider disabled:opacity-50"
        >

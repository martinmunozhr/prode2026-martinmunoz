...
export const Route = createFileRoute("/insights")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Análisis de tus pronósticos: aciertos exactos, parciales, racha y equipos que más prediciste." },
    ],
  }),
...
type DbMatch = {
  id: string; home_id: string; away_id: string; match_date: string;
  stadium: string; city: string; stage: string; group_letter: string | null;
  home_score: number | null; away_score: number | null; status: string;
};

function dbToMatch(m: DbMatch): Match {
  return {
    id: m.id, homeId: m.home_id, awayId: m.away_id, date: m.match_date,
    stadium: m.stadium, city: m.city, stage: m.stage as Match["stage"],
...
function InsightsPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-12">
      <header className="mb-8">
        <div className="text-[11px] uppercase tracking-widest text-primary font-bold">Tus números</div>
        <h1 className="font-display text-5xl md:text-6xl tracking-tight mt-1">Radiografía del Prode</h1>
        <p className="mt-2 text-muted-foreground">Cómo viene tu temporada de pronosticador.</p>
      </header>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
        <Stat icon={<Target />} value={String(insights.totalPoints)} label="Puntos totales" tone="primary" />
        <Stat icon={<Activity />} value={String(insights.totalPreds)} label="Pronósticos" tone="accent" />
        <Stat icon={<Flame />} value={String(insights.currentStreak)} label="Racha actual" tone="primary" />
        <Stat icon={<TrendingUp />} value={String(insights.bestStreak)} label="Mejor racha" tone="accent" />
      </div>

      <section className="mb-10 bg-gradient-card border border-border/50 rounded-2xl p-6">
        <h2 className="font-display text-xl tracking-wider mb-5">Precisión sobre {insights.finishedPreds} partidos cerrados</h2>
        <Bar label="Marcador exacto" pct={insights.exactPct} count={insights.exact} color="bg-pitch" />
        <Bar label="Resultado parcial" pct={insights.partialPct} count={insights.partial} color="bg-primary" />
        <Bar
          label="Errados"
      ...
      <section className="bg-gradient-card border border-border/50 rounded-2xl p-6">
        ...
        {insights.topTeamsPicked.length === 0 ? (
          <p className="text-sm text-muted-foreground">Cargá pronósticos para ver tus tendencias.</p>
        ) : (
...
function Stat({ icon, value, label, tone }: { icon: React.ReactNode; value: string; label: string; tone: "primary" | "accent" }) {
  ...
  return (
    <div className="mb-4 last:mb-0">
      <div className="flex items-center justify-between text-xs mb-1.5">
        ...
        <span className="font-display text-sm tabular-nums">{count} · {pct.toFixed(0)}%</span>
      </div>
